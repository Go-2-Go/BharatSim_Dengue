...
//  All the samples drawn from these distributions represent days
  ...
  final val dt = 1.toDouble / 2.toDouble          // time per unit ticks
  final val inverse_dt = 2.toDouble / 1.toDouble  // ticks per unit time

  final val latentDurationProbabilityDistribution           = LogNormal(10, 2)
  final val mosquitoInfectedDurationProbabilityDistribution = LogNormal(10, 2)
}
